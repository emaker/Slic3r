...
sub make_perimeters {
    ...
    foreach my $surface (@surfaces) {
        ...
        if (1) {
            foreach my $hole ($last_offsets[0]->holes) {
                ...
                my $new_radius = (scale($self->perimeters_flow->width * 0.5) + sqrt((scale($self->perimeters_flow->width)**2) + (4*($radius**2)))) / 2;
    ...
    foreach my $island (@perimeters) {
        ...
        for (reverse 0 .. $#holes) {
        	my $role = $is_external{$_} ? EXTR_ROLE_EXTERNAL_PERIMETER : EXTR_ROLE_HOLE;
        	$self->add_perimeter($holes[$_], $role);
            #print "add hole role is $role \n";        	
        }
        ...
        for my $depth (reverse 0 .. $#$island) {
            ...
            #print "add perimeter role is $role \n";
	        }
    }
